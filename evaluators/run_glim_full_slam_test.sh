...
ros2 run glim_ros glim_rosnode --ros-args \
  ...
  -p dump_path:="$OUTPUT_DIR/dump" >"$OUTPUT_DIR/glim.log" 2>&1 &
glim_pid=$!
sleep 5

ros2 bag record -o "$OUTPUT_DIR/trajectory" \
  /glim_ros/odom /glim_ros/odom_scanend \
  /glim_ros/odom_corrected /glim_ros/odom_scanend_corrected \
  /glim_ros/lidar_odom /glim_ros/lidar_odom_corrected \
  >"$OUTPUT_DIR/record.log" 2>&1 &
record_pid=$!
sleep 2
ros2 bag play "$BAG_DIR" --rate 1.0 >"$OUTPUT_DIR/play.log" 2>&1

# 给 CPU 全局后端时间完成最后一批子图优化。
sleep 30
kill -INT "$record_pid" 2>/dev/null || true
wait "$record_pid" || true
kill -INT "$glim_pid" 2>/dev/null || true

# GLIM 保存完整 dump 可能较慢；最多等待 120 秒，轨迹 bag 已先安全关闭。
