...
ros2 run direct_lidar_inertial_odometry dlio_odom_node --ros-args \
  --params-file "$WORKSPACE/date/dlio_test/dlio.yaml" \
  --params-file "$WORKSPACE/date/dlio_test/params.yaml" \
  -r pointcloud:=/livox/lidar \
  -r imu:=/lio_eval/imu_si \
  ...
  -r deskewed:=/dlio/odom_node/pointcloud/deskewed \
  >"$OUTPUT_DIR/dlio.log" 2>&1 &
dlio_pid=$!
sleep 5

ros2 bag record -o "$OUTPUT_DIR/trajectory" /dlio/odom_node/odom /dlio/odom_node/path \
