...
class SameBagSummaryCliContractTest(unittest.TestCase):
    ...
    def test_finalization_parser_exposes_only_existing_run_input(self) -> None:
        module = self._load_cli()
        args = module.build_parser().parse_args(
            ["summarize", "same-bag-finalize", "--run", "/frozen/full_run"]
        )
        self.assertEqual("summarize", args.command)
        self.assertEqual("same-bag-finalize", args.summarize_command)
        self.assertEqual(Path("/frozen/full_run"), args.run)
        self.assertEqual("cmd_finalize_same_bag", args.func.__name__)
    ...
    def test_finalization_handler_invokes_only_pure_python_evaluator(self) -> None:
        module = self._load_cli()
        calls: dict[str, object] = {}
        original_run_python_ros = module._core.run_python_ros
        original_subprocess_run = module.subprocess.run
        ...
        def forbidden_run_python_ros(*args, **kwargs):
            raise AssertionError("same-bag finalizer must not source ROS or replay a bag")
        ...
        module.subprocess.run = fake_subprocess_run
        try:
            code = module.cmd_finalize_same_bag(SimpleNamespace(run=Path("/frozen/full_run")))
        finally:
            module._core.run_python_ros = original_run_python_ros
            module.subprocess.run = original_subprocess_run
        ...
        self.assertEqual(
            [
                module.sys.executable,
                str(ROOT / "evaluators/finalize_same_bag.py"),
                "--run",
                "/frozen/full_run",
            ],
            calls["command"],
    ...
    def test_evaluators_have_no_ros_or_estimator_execution_dependency(self) -> None:
        for evaluator_name in ("summarize_same_bag.py", "finalize_same_bag.py"):
            with self.subTest(evaluator=evaluator_name):
                evaluator = ROOT / "evaluators" / evaluator_name
